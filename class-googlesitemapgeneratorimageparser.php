<?php
/**
 * XML Sitemap Generator plugin file.
 *
 * @package sitemap
 */
 ...
 */
class GoogleSitemapGeneratorImageParser {
	...
	public function get_images( $post ) {
		...
		/**
		 * Filter: 'sm_sitemap_content_before_parse_html_images' - Filters the post content
		 * before it is parsed for images.
		 *
		 * @param string $content The raw/unprocessed post content.
		 */
		$content = apply_filters( 'sm_sitemap_content_before_parse_html_images', $post->post_content );
		 ...
		 */
		$image_list = apply_filters( 'sm_sitemap_urlimages', $images, $post->ID );
	...
	public function get_term_images( $term ) {
		 ...
		 */
		$image_list = apply_filters( 'sm_sitemap_urlimages_term', $images, $term->term_id );
		if ( isset( $image_list ) && is_array( $image_list ) ) {
			$images = $image_list;
		}
